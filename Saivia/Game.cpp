...
void Game::Initialize(HWND window, int width, int height)
{
	...
	lua_getglobal(ls, "invoke"); //取得lua中的變量
	lua_pushnumber(ls, (double)10); //將第1個參數壓入lua
	lua_pushnumber(ls, (double)20); //將第2個參數壓入lua
	lua_pushnumber(ls, (double)30); //將第3個參數壓入lua
	
	lua_call(ls, 3, 2); //call lua function, 3個參數, 0個回傳數
